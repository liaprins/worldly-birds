...
//        if((swallow_color_code.equals(birds.get(sighting).species) && alpha_test == 255) || (swallow_color_code.equals(birds.get(sighting).species) && hover_swallow_button == true)){
          //  fill(swallow_color);
          
          //  if the button is ON...
          if(swallow_color_code.equals(birds.get(sighting).species) && alpha_test == 255){
          stroke(0);
          fill(100,100,100, alpha_test);
          ellipse(x_position*map_lat, y_position*map_lng,  dot_size_swallow,dot_size_swallow);
          }

          //  if hover when the button is ON
          if(swallow_color_code.equals(birds.get(sighting).species) && alpha_test == 255){
          stroke(0);
          fill(100,100,100, alpha_test);
          ellipse(x_position*map_lat, y_position*map_lng,  dot_size_hover,dot_size_hover);
          }
          
          //  if hover when the button is OFF
          if(swallow_color_code.equals(birds.get(sighting).species) && alpha_test == 255){
          stroke(2);
          noFill();
          ellipse(x_position*map_lat, y_position*map_lng,  dot_size_hover,dot_size_hover);
