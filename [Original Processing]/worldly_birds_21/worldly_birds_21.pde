...
parent file = wb15
 ...
 1. Worldly Birds hover explanation.
 2. Customize fonts for explanation.
 3. Fix counter.
 4. Make no fill for arc lines on buttons.
 5. Try to move galah...?
...
void draw() { 
  ...
  if (millis() > starttime+1000*millis_multiplier) { 
    ...
    fill(clockStroke_R, clockStroke_G, clockStroke_B, caption_alpha);
    text("Which birds go where, and when? Scientists keep track of the species they sight to learn about birds' migratory patterns. Compare the seasonal sightings for this sampling of twenty-five birds by toggling their buttons on and off.", 
        (width - (width*.5 + horizontal_title_offset)), 
        (-height*.5 + clock_offset + monthTextSize + vertical_title_offset) 
         /*
         ,
         width - (width*.5 + horizontal_title_offset) + 200,
        (-height*.5 + clock_offset + monthTextSize + vertical_title_offset + 200)
        */
        );
