...
In this version, I tried to move the illustration to be in front of the sighting dots, but then it looks all jagged-y, even though I draw the background first thing in void draw...
Hover when button is on, illustration is smooth (.png is being drawn before the sighting dots). Hover when button is off, it is jaggedy (illustration drawn after sighting dots).
But when .png has white background ('hummingbird_white.png') it is smooth even when drawn after sighting dots!

Not to mention, it is so slow it would probably crash if I had the patience to let it run for more than a few "days".
...
When I moved the illustration to draw after the sighting dots, it was all jaggedy. When I had it before them, with the rest of the button functionality, it was not jaggedy! (Also slow!)
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% INDIVIDUAL SPECIES  
  ...
  hummingbird_img = loadImage("hummingbird_map.png");
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% INDIVIDUAL SPECIES
    //  HOVER   
    //  if NOT hovering over species' button && the button is OFF... JUST THE IMAGE + LINE
//    if(hover_swallow_button == false){                
//    }
    //  if YES hovering over species' button && the button is ON...
    if(hover_swallow_button == true && swallow_dot_alpha == alpha_on){      
...
//  /* v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v
    //  if YES hovering over species' button && the button is OFF... JUST THE IMAGE + LINE    
    if (hover_swallow_button == true && swallow_dot_alpha == alpha_off){
      strokeWeight(line_stroke_weight);
      stroke(clockStroke_R, clockStroke_G, clockStroke_B, clockStroke_Alpha);      
      line(((width - 2*button_wall_offset) / 24)*(-1 + 1)  //  <<<<<< REPLACE WITH INDIVIDUAL SPECIES NUMBER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
             + button_wall_offset - width*.5,
             height - button_vertical_offset - (button_width*.5 - height*.5),
           ((width - 2*button_wall_offset) / 24)*(-1 + 1)  //  <<<<<< REPLACE WITH INDIVIDUAL SPECIES NUMBER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
             + button_wall_offset - width*.5,
             height - button_vertical_offset - (button_width*.5) - line_height - height*.5);  
      arc(((width - 2*button_wall_offset) / 24)*(-1 + 1)  //  <<<<<< REPLACE WITH INDIVIDUAL SPECIES NUMBER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            + button_wall_offset + (line_diameter*.5) - width*.5,
            height - button_vertical_offset - (button_width*.5) - line_height - height*.5,  
            line_diameter,line_diameter,  radians(180), radians (270));      
       line(((width - 2*button_wall_offset) / 24)*(-1 + 1)  //  <<<<<< REPLACE WITH INDIVIDUAL SPECIES NUMBER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
              + button_wall_offset + line_diameter*.5 - width*.5,
              height - button_vertical_offset - (button_width*.5) - line_height - line_diameter*.5 - height*.5,      
            ((width - 2*button_wall_offset) / 24)*(-1 + 1)  //  <<<<<< REPLACE WITH INDIVIDUAL SPECIES NUMBER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
              + button_wall_offset + button_width + hummingbird_img.width*scale - line_diameter - width*.5,
              height - button_vertical_offset - (button_width*.5) - line_height - line_diameter*.5 - height*.5);        
      image(hummingbird_img,
            ((width - 2*button_wall_offset) / 24)*(-1 + 1)  //  <<<<<< REPLACE WITH INDIVIDUAL SPECIES NUMBER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
              + button_wall_offset + img_horizontal_offset - width*.5,
              height - (button_vertical_offset) - hummingbird_img.height - line_height - line_diameter + img_vertical_offset - height*.5, 
              hummingbird_img.width*scale, hummingbird_img.height*scale);
      }
//  ^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^ */        
