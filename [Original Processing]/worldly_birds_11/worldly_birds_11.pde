...
//  ALL SPECIES: dot sizes, no hover, for button on
float dot_size_swallow = dot_size;

//  ALL SPECIES: dot alpha, for hover & on/off
int swallow_dot_alpha = alpha_on;

//  ALL SPECIES: dot stroke alpha, for hover & on/off
int swallow_stroke_alpha = alpha_on;

//  ALL SPECIES: colors for dots
color swallow_color = color(27,117,188,  swallow_dot_alpha);

//  ALL SPECIES: boolean hover for button
boolean hover_swallow_button = false;

//  /* v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v
//  ALL SPECIES: colorcodes (string 1) to determine species, dot colors
    ...
    for(int sighting = 0; sighting<birds.size(); ++sighting){ 
      ...
      if((counter>=date_projection - sighting_duration && 
        ...
        if((swallow_color_code.equals(birds.get(sighting).species) && swallow_dot_alpha == alpha_on) || (swallow_color_code.equals(birds.get(sighting).species) && hover_swallow_button == true)){
          //  fill(swallow_color);
          stroke(255,0,0, swallow_stroke_alpha);
          strokeWeight(2);
          fill(100,100,100, swallow_dot_alpha);
