...
RGBs TO BE REFERENCED WHEN BUILDING LIST BELOW IN VOID DRAW
...
//  color swallow_color = color(27,117,188,  dot_alpha);
...
//%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% INDIVIDUAL SPECIES
    ...
    if(hover_swallow_button == true && swallow_dot_alpha == alpha_on){      
      ...
      swallow_stroke_alpha = alpha_off;      
      image(hummingbird_img,  hummingbird_img.width,hummingbird_img.height, hummingbird_img.width*scale, hummingbird_img.height*scale);
    ...
    stroke(27,117,188,  swallow_button_stroke_alpha);
    strokeWeight(button_stroke_weight);
    fill(27,117,188,  swallow_button_fill_alpha);
    ellipse((((width - 2*button_wall_offset) / 25)*1)  //  <<<<<< REPLACE WITH INDIVIDUAL SPECIES NUMBER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                 + button_wall_offset + button_width,
               height - (button_vertical_offset + (button_width*.5)),  button_width, button_width);
...
//      ellipseMode(CORNER);
      ellipse((((width - 2*(button_wall_offset)) / 25)*(button_outline_number)) 
                    + button_wall_offset + button_width,
                  height - (button_vertical_offset + (button_width*.5)),  button_width, button_width);
      //  re-setting ellipse mode back to normal...
      ellipseMode(CENTER);
      }
    ...
    for(int sighting = 0; sighting<birds.size(); ++sighting){ 
      ...
      if((counter>=date_projection - sighting_duration && 
        ...
        if((swallow_color_code.equals(birds.get(sighting).species) && swallow_dot_alpha == alpha_on) || (swallow_color_code.equals(birds.get(sighting).species) && hover_swallow_button == true)){
          stroke(27,117,188,  swallow_stroke_alpha);
          strokeWeight(dot_stroke_weight_hover);
          fill(27,117,188,  swallow_dot_alpha);          
...
boolean swallow_hover( int x_swallow, int y_swallow){
  if(dist(((width - 2*button_wall_offset) / 25)*1  //  <<<<<< REPLACE WITH INDIVIDUAL SPECIE'S NUMBER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
              + button_wall_offset + button_width,
            height - (button_vertical_offset + (button_width*.5)),  mouseX, mouseY)*2 < button_width) {
    return true;
    } else { 
      return false; 
      }  
...
void mouseClicked(){
  ...
  if(dist(
    ((width - 2*button_wall_offset) / 25)*1  //  <<<<<< REPLACE WITH INDIVIDUAL SPECIES NUMBER %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        + button_wall_offset + button_width,
