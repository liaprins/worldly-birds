...
0. Organize all information and ints on this one!
...
//  INDIVIDUAL SPECIES: dot color
//  INDIVIDUAL SPECIES: stroke color
...
//  INDIVIDUAL SPECIES: colors for dots
...
//  /* v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v-v    
    ...
    if(hover_swallow_button == false){
      ...
      println("false, & no hover");
                
        //  ...sighting dots are normal size...  
        dot_size_swallow = dot_size;
        
        //  ...and no stroke (WHY WON'T THIS WORK???)
      swallow_stroke_alpha = 0;
      noStroke();
    ...
    if(hover_swallow_button == true && swallow_dot_alpha == alpha_on){
      ...
      //  ...species' sighting dots gets bigger 
      dot_size_swallow = dot_size_hover;
      
      //  ...and no stroke (WHY WON'T THIS WORK EITHER???)
      swallow_stroke_alpha = 0;
      
      //  ...species' illustration appears
    ...
    //  draw button ('19' is swallow's number), how many buttons over to the right it's button is
    fill(swallow_color);
    ellipse((((width - 2*button_wall_offset) / 25)*19), 
...
//  ^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^-^ */
    ...
    for(int sighting = 0; sighting<birds.size(); ++sighting){ 
      ...
      if((counter>=date_projection - sighting_duration && 
        ...
        //  if this species' name is in column 1 of Excel spreadsheet...
        if((swallow_color_code.equals(birds.get(sighting).species) && swallow_dot_alpha == alpha_on) || (swallow_color_code.equals(birds.get(sighting).species) && hover_swallow_button == true)){
          ...
          //  ...draw the swallow dots at the specified lat and lng
...
boolean swallow_hover( int x_dodo, int y_dodo){
  ...
  if(dist(
    ((width - 2*button_wall_offset) / 25)*19, 
    height - (button_vertical_offset + (button_width*.5)),
    mouseX, mouseY)*2 
    < button_width) {
...
void mouseMoved(){
    ...
    //  I haven't noticed that I need to call redraw... but maybe...?
    //redraw();   
